...
class Trainer(object):
    def __init__(self, args):
        ...
        model = AutoDeeplab (num_classes=self.nclass, num_layers=12, criterion=self.criterion, filter_multiplier=self.args.filter_multiplier,
                             block_multiplier_c=self.args.block_multiplier, step_c=self.args.step)
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            output, latency = self.model(image)
            ...
            if epoch >= self.args.alpha_epoch:
                ...
                output_search, latency = self.model(image_search)
                arch_loss = self.criterion(output_search, target_search) + latency
                if self.use_amp:
...
def main():
    ...
    parser.add_argument('--arch-lr', type=float, default=6e-3, metavar='LR',
